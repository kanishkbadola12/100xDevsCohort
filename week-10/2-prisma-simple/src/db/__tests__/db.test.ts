...
describe('User Database Operations', () => {
    ...
    test('getUser retrieves a user by ID', async () => {
        ...
        const newUser = await createUser('newuser', 'password', 'New User') as any;
...
describe('Todo Operations', () => {
    ...
    beforeAll(async () => {
      ...
      const user = await createUser('todouser', 'password', 'Todo User') as any;
      userId = user.id;
    });
    ...
    test('updateTodo marks a todo as done', async () => {
      const todo = await createTodo(userId, 'Update Test', 'To be updated') as any;
    ...
    test('getTodos retrieves all todos for a user', async () => {
      ...
      const todos = await getTodos(userId) as any;
